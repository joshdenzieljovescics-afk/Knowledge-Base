...
# NOTE: ALL imports that might trigger circular imports are done LAZILY inside functions
# This includes: core.pdf_extractor, config, services.lambda_image_service
...
def parse_and_chunk_pdf_file(file_bytes, source_filename):
    ...
    """
    # LAZY IMPORTS - All imports that could cause circular imports are done here
    ...
    # In Lambda, use dedicated image processor Lambda for better performance
    if Config.IS_LAMBDA and is_design_heavy:
        print("[DEBUG] Using Lambda image processor for design-heavy PDF")
        try:
            from services.lambda_image_service import get_lambda_image_service

            image_service = get_lambda_image_service()
            # Extract images from all pages
            images = image_service.extract_images_from_pdf(file_bytes)
            print(f"[DEBUG] Lambda image processor returned {len(images)} images")
        except Exception as e:
            print(f"[ERROR] Lambda image processor failed: {e}")
            # Fall back to extracting from structured elements
        ...
        # Local mode or non-design-heavy: extract from structured elements
        for el in structured:
            if el.get("type") == "image" and el.get("image_b64"):
                images.append(
                    {
    ...
    merged_result = merge_text_and_image_chunks(
        text_result, image_result, simplified_view, structured, source_filename
    )
    ...
    anchored_chunks = anchor_chunks_to_pdf(merged_result.get("chunks", []), structured)
    ...
    final_result = {
        ...
        "document_metadata": {
            ...
            "anchored_chunks": sum(
                ...
                if chunk.get("metadata", {}).get("anchored", False)
            ),
            "unanchored_chunks": sum(
                1
                for chunk in anchored_chunks
                if not chunk.get("metadata", {}).get("anchored", False)
            ),
        ...
        "extraction_summary": {
            ...
            "table_elements": len(
                [el for el in structured if el.get("type") == "table"]
            ),
...
class PDFService:
    ...
    async def process_pdf(
        self, pdf_content: bytes, document_id: str, filename: str, user_id: str
    ):
        """Process PDF and return chunks."""
        result = parse_and_chunk_pdf_file(pdf_content, filename)
